...
class ShortRepository extends repository_base_1.default {
    ...
    async listCommentsByShortId(shortId, userId) {
        const query = `
        SELECT c.id,
               c.content,
               c.created_at,
    ...
    async deleteShortComment(commentId, userId) {
        ...
        const deleteQuery = `DELETE
                         ...
                         WHERE id = $1
                           AND user_id = $2`;
        try {
            const short = await this.database.query(selectQuery, [commentId]);
            if (!short.rowCount) {
