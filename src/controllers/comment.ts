...
export class CommentController extends BaseController<CommentRepository> {
	...
	public async listByPublication(request: Request<{ publicationId: string }>, response: Response<T.Comment.Populated[]>) {
	...
	public async create(request: Request<{ publicationId: string }, object, { content: string }>, response: Response<T.Comment.Comment>) {
		this.log.log('create');
		try {
			const publicationId = Validate.uuid.parse(request.params.publicationId);
			const userId = Validate.uuid.parse(request.user.id);
	...
	public async delete(request: Request<{ commentId: string }>, response: Response<void>) {
		this.log.log('delete');
		try {
	...
	public async like(request: Request<{ commentId: string }>, response: Response<void>) {
		this.log.log('like');
		try {
			const commentId = Validate.uuid.parse(request.params.commentId);
			const userId = Validate.uuid.parse(request.user.id);
