	...
	if (token) {
		try {
			...
			if (user) {
				...
				return response.status(statusCodes.FORBIDDEN).json({message: 'User is already authenticated'});
			}
		} catch (error: unknown) {
...
export function  isAuthorized(request: Request, response: Response, next: NextFunction) {
	...
	if (!token) {
		...
		return response.status(statusCodes.UNAUTHORIZED).json({message: 'User is not authenticated'});
	}
