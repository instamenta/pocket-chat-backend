...
export class ShortRepository extends BaseRepository {
    ...
    try {
      ...
      if (likeExistsResult.rows.length > 0) {
        const removeLikeQuery =
          "DELETE FROM short_likes WHERE short_id = $1 AND user_id = $2";
        ...
        const addLikeQuery =
          "INSERT INTO short_likes (short_id, user_id) VALUES ($1, $2)";
  ...
  public async deleteShortComment(
    ...
    const deleteQuery = `DELETE
                         ...
                         WHERE id = $1
                           AND user_id = $2`;
    try {
      const short = await this.database.query<{ short_id: string }>(
        selectQuery,
        [commentId],
      );
      if (!short.rowCount) {
  ...
  public async likeShortComment(
    ...
    try {
      ...
      if (likeExistsResult.rows.length > 0) {
        const removeLikeQuery =
          "DELETE FROM short_comment_likes WHERE comment_id = $1 AND user_id = $2";
        await this.database.query(removeLikeQuery, [commentId, userId]);
      } else {
        const addLikeQuery =
          "INSERT INTO short_comment_likes (comment_id, user_id) VALUES ($1, $2)";
