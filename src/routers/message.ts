...
export default class MessageRouter extends BaseRouter<MessageController> {
	initialize(c: MessageController) {
		...
		// @ts-expect-error - to assign handlers
		this.router.put('/:id', Middlewares.isAuthorized, c.updateMessageStatus.bind(c));

		// @ts-expect-error - to assign handlers
		this.router.get('/conversations', Middlewares.isAuthorized, c.listConversations.bind(c));
		// @ts-expect-error - to assign handlers
